...
__global__ void CR_GPU(float *A, float *B, float *C, float *D, float *X, const unsigned int n) {
    ...
    if (idx == k) { X[k] = (sB[l] * sD[k] - sC[k] * sD[l]) / s1; }
    if (idx == l) { X[l] = (sD[l] * sB[k] - sD[k] * sA[l]) / s1; }
    __syncthreads();
    ...
    for (int i = 0; i < ln - 1; i++) {
        if (idx == k) {
            X[k] = (sD[k] - sC[k] * X[k + step]) / sB[k];
        }
        __syncthreads();

        if (idx >= k && idx < n && !((idx - k) % stride)) {
            X[idx] = (sD[idx] - sA[idx] * X[idx - step] - sC[idx] * X[idx + step]) / sB[idx];
        }
...
int main(int argc, char *argv[]) {
    ...
    // GPU
    timestamp(&start);
    CR_GPU<<<dimGrid, dimBlock, 4 * numBytes>>>(dA, dB, dC, dD, dX, n);
    ...
    cudaMemcpy(hX, dX, numBytes, cudaMemcpyDeviceToHost);
    for (int i = 0; i < n; i++) {
